...
CRITICAL: The hero's journey is a structural framework, not a mandate for a happy ending. The tone and ending of the story are dictated entirely by the intro prompt and the metadata tone — not by any convention of the hero's journey. A tragic story uses the same 9 steps and ends in loss, death, destruction, or despair. A bleak story climaxes in confrontation and resolves in ruin. The hero's journey does not promise redemption, survival, love, or success. It only provides structure. Override any default assumption toward a positive ending if the tone, intro prompt, or genre calls for something darker.
  ...
  4. rising_complications — the central conflict escalates through a series of connected obstacles, each harder than the last. This is not a single block of rising action — it is a sequence of discrete complications, each with its own time gap, trigger, and cost. Three is a REQUIRED FLOOR, not the rule — it is the minimum for the shortest stories, not a target to stop at. This is the largest single section of the spine (32% of word count) and should have a complication count that reflects that weight. Scale the count to story length: under 30k words, at least 3; 30k–60k words, at least 5–6; 60k–90k words, at least 7–9; above 90k words, at least 1 complication per 10k words. When in doubt, add more rather than fewer — a thin rising_complications section starves the middle of the book.

     Complications must be WOVEN, not stacked. A complication resolving cleanly before the next one starts is a failure of this requirement — do not write "A happens and resolves, then B happens and resolves, then C happens and resolves." Instead, most complications should open before the previous one has closed: B should erupt while A is still unresolved, and A's fallout should still be live and complicating things when C hits. Each new complication should reach back and entangle with at least one earlier, still-open complication — reopening it, making it worse, forcing a choice between it and the new problem, or colliding the two together — rather than existing as its own clean, self-contained unit. By the middle of this section, the protagonist should be juggling multiple live, tangled problems at once, not resolving them one at a time in a queue. In the summary field, name each complication explicitly in its own paragraph: what happens, what drives it, what it costs the protagonist, and which earlier complication(s) it entangles with or reopens. Every complication must trace back to the same central conflict — no unrelated subplots. The forbidden element must actively create or worsen at least two of these complications. Time gaps between complications are expected and must be stated explicitly for each one — life continues between story events, but "time gap" does not mean the previous complication is resolved by the time the next one starts.

     Before returning the spine, count the complications explicitly and verify the count meets or exceeds the minimum for this word count — if it does not, add more before returning. Also verify that at least half of the complications entangle with an earlier one rather than standing alone — if the section reads as a clean sequential list, rewrite it so the problems overlap and compound.
  ...
  8. climax — the protagonist confronts the central conflict directly, through their own agency, and the immediate outcome of that direct confrontation lands here — victory, defeat, death, or something more ambiguous, whatever is consistent with the story's tone. This is not automatically a triumph; in a tragedy, the climax is where the protagonist's flaw destroys them, or they make the choice that seals their fate. Treat this as the outcome as the protagonist and reader experience it in the moment — final, as far as anyone in the room knows.
  9. resolution — where the story's true, lasting outcome lands, which is not always identical to what the climax appeared to settle. If nothing overturns the climax, resolution is simply its fallout: what changed, what was won or lost, where every significant character lands. But if the metadata or intro prompt calls for the climax's apparent outcome to be undercut, revoked, or rendered hollow by something outside the protagonist's control — a decision-maker, a rigged process, information the protagonist didn't have — that reversal belongs here, not at the climax and never earlier. The climax's outcome must be allowed to stand as real and believed, however briefly, before resolution recontextualizes it; do not foreshadow the reversal so explicitly in earlier plot points that it stops landing as a gut-punch when it arrives here. The forbidden element must be addressed: either resolved, paid for, or accepted with consequences. The resolution is a landing, not a coda — it closes the story with a single decisive beat, not a chapter of characters reflecting on what happened. No rumination, no drawn-out emotional processing, no epilogue energy. If the story's tone is tragic, bleak, or bitter, the resolution must reflect that — no false hope, no softening, no redemptive consolation unless the intro prompt specifically calls for one.
...
Additional rules:
  ...
  - This subordination rule applies to every non-protagonist character, not only the antagonist — a rival, love interest, or any other secondary character whose arc is written to "win," to be vindicated, or to be proven right must still be experienced through what that outcome does to the protagonist, not as an independent thread the spine hands its narrative weight to. If a secondary character's want is designed to triumph over the protagonist's, that triumph is a plot point that acts ON the protagonist — advancing or breaking their arc — not a parallel success story told for its own sake. The protagonist stays the fixed point-of-view center and structural throughline across all 9 steps; no other character's arc may absorb that role partway through, regardless of who is right, who is more sympathetic, or who the story's thesis favors.
...
Map the romantic and sexual tension arc across the plot points you are generating. For each plot point note whether the intimate arc role is tension-building, escalation, payoff, or none. The forbidden element must be actively creating pressure in every tension-building and escalation plot point — not just present, but felt as a reason this cannot or should not happen. Early plot points should establish desire and chemistry through proximity and awareness. Escalation plot points use near-misses, interrupted moments, and unresolved longing to build pressure. The first payoff must be earned — it requires at least one tension-building plot point and one escalation plot point before it — but it can arrive as soon as the buildup justifies it. Do not artificially delay it past the point where it has been earned.

Payoff plot points are a REQUIRED minimum, not an optional maximum — a single payoff across the whole spine is a failure of this requirement regardless of word count. This is meant to be a story saturated with romance and physical intimacy — payoff should be frequent, not rationed. More payoffs are always better than fewer, as long as each one is earned and distinct: when in doubt, err toward more. Scale the minimum payoff count to story length: under 30k words, at least 4 payoffs; 30k–60k words, at least 6–8 payoffs; 60k–90k words, at least 9–12 payoffs; above 90k words, at least 1 payoff per 8k words. Treat these as floors, not targets — exceed them whenever the story's buildup supports it. Distribute payoffs across the back three-quarters of the spine (crossing_threshold onward) rather than clustering them near the climax — space them so each one raises the stakes, shifts the dynamic, or deepens the relationship rather than repeating the previous one. Payoff plot points must deliver fully — nothing withheld, nothing faded-to-black unless the metadata specifies otherwise. Before returning the spine, count the payoff plot points explicitly and verify the count meets or exceeds the minimum for this word count — if it does not, add more before returning.

Do not let payoff plot points settle into a single repeating shape of "plot event happens, then intimacy follows as a separate beat" — across many payoffs in sequence that pattern becomes mechanical and predictable. Vary the relationship between the plot event and the intimacy within each payoff's summary: sometimes the intimacy should be the direct, immediate consequence of what the plot event just caused; sometimes it should erupt in the middle of the plot event itself, before it resolves, forcing the characters to navigate finishing what they were doing while hiding what's happening between them; sometimes a payoff plot point should open already mid-encounter, with the reader learning what led here or what they're risking by continuing only after the fact; sometimes there should be no clean plot trigger at all, just proximity and unresolved history catching up with them. Look back at how the last two or three payoffs were structured before writing the next one, and deliberately choose a different shape.
...
Beyond those, invent a couple of additional items appropriate to this world, genre, and setting. Think broadly: what objects exist in this world and what do people carry, own, use, trade, hide, lose, or fight over? Draw from the specific world and tone — the items of a gothic manor differ from those of a space station differ from those of a Regency ballroom. Include objects that are mundane but specific, objects that carry personal history, objects that exist for atmosphere. Key items get full detail. Minor objects get a brief note.
