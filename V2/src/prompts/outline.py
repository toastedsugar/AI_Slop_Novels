...
{{
  ...
  "organization_stubs": ["One paragraph stub per organization or faction that plays a role in the story."],
  "event_stubs": ["One sentence stub per key story event — the what and when."],
  "chapters": [
    {{
      "chapter_number": 1,
      "title": "Chapter title",
...
Your purpose is to expand the character, location, item, organization, and event stubs from the rough outline into full detail. Use the stubs as your foundation — do not invent characters, locations, items, organizations, or events that are not already present in the outline. Do not change the structure in any way besides modifying the pacing. If there is a specific amount of chapters, stay true to that. On a beat by beat and scene by scene basis, you are free to make adjustments to guarantee the strongest possible pacing.
...
Every object must have a unique uuid-v4 as its id. All cross-reference fields (initial_holder, initial_location, characters_involved, organizations_involved) must contain the actual uuid of the object they reference, not a placeholder.
...
There will also likely be alot of vagueness in the rough outline, especially in the chapter specific information. Every chapter must have a clear purpose in the overall story arc — remove filler. Make sure the tension level varies across chapters so the story breathes and does not flatline. Make sure to confirm that character dialogue matches their voice and personality.

Chapters can be anywhere from 0 to 2000 words. Prioritize variety in chapter length — a very short punchy chapter after a long one is a valid and effective pacing tool. Let the story dictate the length, not a target number.

A beat is a discrete unit of narrative action — one thing happening. It has no fixed length. It could be a single paragraph of a character brooding in silence, a long drawn-out dialogue scene, a moment of physical tension, an action sequence, or a page of exposition. What makes it a beat is that it is one coherent thing with a single purpose. A chapter will typically have 3-8 beats.

Each beat must be concrete and specific — not "they talk" but what is said, why it matters, and what shifts as a result. The prose generator uses each beat as a direct instruction, so there must be no ambiguity about what happens, who is present, where they are, and what the emotional and physical stakes are.

For any beat where `heat_level` is charged or explicit, the `description` must describe the intimate content explicitly and in physical terms — the actual progression of what happens, nothing abstracted or implied.

For each chapter and its beats provide a detailed breakdown using the schema below. Do not stray from this schema. Chapters and beats must be returned as two separate top-level arrays. Each beat must set its chapter_id to the uuid of the chapter it belongs to.

{schema}

Every object must have a unique uuid-v4 as its id. All cross-reference fields (chapter_id, pov, location_id, characters_present_ids, items_present_ids) must contain the actual uuid of the object they reference, not a placeholder. Validate that all referenced uuids exist in the character, location, and item sheets. Return only the JSON object, no other text.
